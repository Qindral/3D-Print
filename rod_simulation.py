...
def draw_rods(screen: pygame.Surface, rods: List[Rod], angle_x: float, angle_y: float) -> None:
    screen.fill(BACKGROUND_COLOR)
    scale = screen.get_width() / (CUBE_SIZE * 0.8)
    for rod in rods:
        endpoints = rod.endpoints()
        color = CONNECTED_COLOR if rod.connections else ROD_COLOR
        point_a = project_point(endpoints["A"], angle_x, angle_y, scale, screen.get_size())
        point_b = project_point(endpoints["B"], angle_x, angle_y, scale, screen.get_size())
