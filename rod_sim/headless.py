...
def _update_status(fig, ui_state: HeadlessUIState, message: str) -> None:
    ...
    fig._status_text.set_text(f"Status: {message}")
...
def run_headless_phase(
    ...
    fig._status_text = fig.text(0.02, 0.02, f"Status: {ui_state.status_message}")  # type: ignore[attr-defined]
    ...
    _install_controls(fig, ui_state, state_queue, stop_event, current_snapshot)
    ...
    while plt.fignum_exists(fig.number) and not stop_event.is_set():
        ...
        if updated:
            ...
            ui_state.history_free.append(current_snapshot.free_end_count)
        ...
        if updated and now - last_chart_update >= 0.05:
            ...
            if sizes:
                ...
                if bins:
                    ax_hist.set_xlim(0.5, max(bins) + 0.5)
                if len(bins) <= 20:
